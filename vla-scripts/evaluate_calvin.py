...
class _NoOpVideoClip:
    def write_videofile(self, *args, **kwargs) -> None:
        return None


def make_video_clip(frames, fps: int):
    return ImageSequenceClip(frames, fps=fps) if SAVE_ROLLOUT_VIDEOS else _NoOpVideoClip()
...
class GenerateConfig:
    ...
    use_pro_version: bool = True                # Match the released CALVIN-ABC-Pro checkpoint

    use_l1_regression: bool = True                   # If True, uses continuous action head with L1 regression objective
    use_diffusion: bool = False                      # If True, uses continuous action head with diffusion modeling objective (DDIM)
    use_flow_matching: bool = False                  # If True, uses flow matching action head
    flow_matching_head_type: str = "ditx"            # "ditx" | "mlp"
    dit_num_blocks: int = 12
    dit_num_inference_steps: int = 5
    dit_num_inference_samples: int = 1
    dit_pure_inference: bool = True
    dit_disable_inference_anchor: bool = True
    ...
    save_rollout_videos: bool = False                # Disable for standard 1000-sequence metric runs
    ...
    num_sequences: int = 1000                        # CALVIN benchmark sequences (1000 for the full benchmark)
    ep_len: int = 360                                # Maximum environment steps per subtask
...
#################################################################################################################
    ...
    calvin_dataset: str = "task_ABC_D"               # Use calvin_debug_dataset for a lightweight smoke test
    log_dir: str = "log"
    ...
    for step in range(ep_len):
        ...
        if len(current_task_info) > 0:
            ...
            for key in img_dict.keys():
                clip = make_video_clip(img_dict[key], fps=50)
...
def _load_dataset_stats(vla: torch.nn.Module, checkpoint_path: str) -> None:
    ...
    if os.path.isfile(dataset_statistics_path):
        ...
        # Training RLDS statistics may be saved under the TFDS name while the
        # CALVIN rollout wrapper uses the historical *_rlds alias.
        if "calvin_abc_rlds" not in norm_stats and "calvin_abc" in norm_stats:
            norm_stats["calvin_abc_rlds"] = norm_stats["calvin_abc"]
...
def main(cfg: GenerateConfig):
    global CALVIN_ROOT, SAVE_ROLLOUT_VIDEOS
    CALVIN_ROOT = os.path.abspath(os.path.expanduser(cfg.calvin_path))
    ...
    # Fine-tuned checkpoints keep CALVIN normalization statistics alongside the
    # adapter weights.  Load them explicitly before the rollout wrapper reads
    # `model.norm_stats`; otherwise CALVIN evaluation fails with a missing
    # `calvin_abc_rlds` key even when dataset_statistics.json is present.
    ...
    env = make_env(os.path.join(CALVIN_ROOT, "dataset", cfg.calvin_dataset), observation_space, DEVICE)
