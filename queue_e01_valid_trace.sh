...
cd "$REPO_DIR"
while pgrep -f '[r]un_e01_softprob_retrace.sh' >/dev/null; do sleep 30; done
mkdir -p experiment_results/skill_depth/E01_valid
...
"$PY" experiments/robot/libero/run_libero_eval.py \
  ...
  --center_crop True --seed 9 --run_id_note E01_valid_trace \
  --routing_trace_enabled True --routing_trace_path experiment_results/skill_depth/E01_valid/trace.jsonl \
  > experiment_results/skill_depth/E01_valid/eval.log 2>&1
date --iso-8601=seconds > experiment_results/skill_depth/E01_valid/complete.marker
