...
import requests
import torch
# Initialize the PyTorch CUDA runtime before TensorFlow is imported.  On the
# A100 host, importing TensorFlow first can bind an incompatible CUDA shim and
# make subsequent torch.cuda.set_device() report a spurious old-driver error.
try:
    if torch.cuda.is_available():
        torch.cuda.get_device_name(0)
except Exception:
    pass
# TensorFlow is used for data utilities only.  Keep it on CPU during import so
# it cannot bind a conflicting CUDA shim before the PyTorch training process
# initializes the A100 context.
_cuda_visible_before_tf = os.environ.get("CUDA_VISIBLE_DEVICES")
os.environ["CUDA_VISIBLE_DEVICES"] = ""
import tensorflow as tf
if _cuda_visible_before_tf is None:
    os.environ.pop("CUDA_VISIBLE_DEVICES", None)
else:
    os.environ["CUDA_VISIBLE_DEVICES"] = _cuda_visible_before_tf
...
def get_action_head(cfg: Any, llm_dim: int, model: Optional[torch.nn.Module] = None) -> Union[L1RegressionActionHead, DiffusionActionHead, FlowMatchingMLPActionHead, torch.nn.Module]:
    ...
    if cfg.use_flow_matching:
        ...
        if head_type == "mlp":
            ...
            action_head = FlowMatchingMLPActionHead(
                ...
                anchor_rotation_weight=getattr(cfg, "flowmlp_anchor_rotation_weight", 1.0),
                ...
                dense_film_state_mode=getattr(cfg, "flowmlp_dense_film_state_mode", "full"),
            )
        else:
            ...
            action_head = FlowMatchingActionHead(
                ...
                dense_film_enabled=getattr(cfg, "dit_dense_film_enabled", False),
                dense_film_max_layers=getattr(cfg, "dit_dense_film_max_layers", 64),
                dense_film_first_layer_index=getattr(cfg, "dit_dense_film_first_layer_index", 1),
                dense_film_bottleneck_dim=getattr(cfg, "dit_dense_film_bottleneck_dim", 64),
                dense_film_state_dim=getattr(cfg, "dit_dense_film_state_dim", 128),
                use_adaptive_bridge=getattr(cfg, "use_adaptive_bridge", True),
                bridge_mode=getattr(cfg, "bridge_mode", "adaptive"),
                fixed_layer_index=getattr(cfg, "fixed_layer_index", -1),
            )
    ...
    if not skip_loading:
        try:
            if (
                ...
                and getattr(cfg, "flowmlp_dense_film_enabled", False)
            ):
                incompatible = action_head.load_state_dict(state_dict, strict=False)
                disallowed_missing = [
                    key for key in incompatible.missing_keys
                    if not key.startswith("dense_depth_film.")
                ]
                if disallowed_missing or incompatible.unexpected_keys:
                    raise RuntimeError(
                        "Incompatible FlowMLP dense-FiLM checkpoint: "
    ...
    if hasattr(action_head, "set_routing_step"):
        # Preserve the router-temperature schedule at the checkpoint's actual
        # optimizer step.  Using the terminal temperature for an intermediate
        # checkpoint (or zero when the schedule is absent from eval flags)
        # changes query_key_soft routing at inference.
        routing_step = None
        if not model_is_on_hf_hub(cfg.pretrained_checkpoint):
            try:
                state_path = find_checkpoint_file(cfg.pretrained_checkpoint, "training_state")
                state_name = os.path.basename(state_path)
                routing_step = int(state_name.split("--", 1)[1].split("_checkpoint", 1)[0])
            except (AssertionError, IndexError, ValueError):
                pass
        if routing_step is None:
            routing_step = int(getattr(cfg, "flowmlp_skill_temperature_anneal_steps", 0))
        action_head.set_routing_step(routing_step)
        print(f"[compat] FlowMLP routing step for evaluation: {routing_step}")
