...
LABEL=dit12_h896_std_action_prefix_pure_frommlp5000_dit5000_a100_lr1e4_b8ga2_20260710
RUN_ID=DIT12-h896-std-action-prefix-pure-frommlp5000-dit5000-a100-lr1e4-b8ga2-20260710
...
wait_gpu(){
  while true; do
    ...
    if [ "$free" -gt 60000 ] && [ "$jobs" -eq 0 ]; then break; fi
    sleep 60
  done
}
...
"$TORCHRUN" --standalone --nnodes 1 --nproc_per_node 1 \
  vla-scripts/finetune.py \
    ...
    --run_id_note "$RUN_ID" --run_id_override configs+libero_object_no_noops+b8+lr-0.0001+lora-r32+dropout-0.0--image_aug--${RUN_ID} \
