...
LABEL=dit12_h896_std_action_prefix_pure_frommlp5000_continue10000to12000_v48_b8ga2_20260709
RUN_ID=DIT12-h896-std-action-prefix-pure-frommlp5000-continue10000to12000-v48-b8ga2-20260709
TRAIN_LOG=train_logs/${LABEL}.log
QUEUE_LOG=train_logs/queue_${LABEL}.log
STATUS=train_logs/${LABEL}.status
RESUME_CKPT=/autodl-fs/data/skill_depth/code/action_expert_prefix_trial/outputs/configs+libero_object_no_noops+b8+lr-0.0002+lora-r32+dropout-0.0--image_aug--DIT12-h896-std-action-prefix-pure-frommlp5000-resume5000to10000-v48-b8ga2-20260709--10000_chkpt
...
if [ -f "$STATUS" ] && [ "$(cat "$STATUS")" = 0 ]; then
  ...
  : > "$TRAIN_LOG"
  : > "$STATUS"
  log "launch DIT continue 10000 -> 12000 label=$LABEL"
  wait_gpu
  set +e
...
if [ -n "$chkpt" ]; then
  ...
  log "missing checkpoint step=12000"
  exit 1
fi
