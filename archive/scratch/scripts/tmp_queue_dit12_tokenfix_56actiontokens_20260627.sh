...
OLD_CKPT=outputs/configs+libero_object_no_noops+b16+lr-0.0002+lora-r32+dropout-0.0--image_aug--VLA-Adapter-DIT12-pure-uniformt-nodetach1500-20260624--1000_chkpt
RUN_ID=configs+libero_object_no_noops+b16+lr-0.0002+lora-r32+dropout-0.0--image_aug--VLA-Adapter-DIT12-tokenfix56-pure-uniformt-nodetach1500-20260627
QUEUE_LOG=train_logs/queue_dit12_tokenfix56_pure_uniformt_nodetach1500_20260627.log
TRAIN_LOG=train_logs/dit12_tokenfix56_pure_uniformt_nodetach1500_20260627.log
...
run_fixed_inference_probe() {
  local log verify_log successes
  for seed in 0 3 7; do
    log="train_logs/dit12_tokenfix56_oldckpt_fixedinfer_steps20_seed${seed}_eval3_20260627.log"
    echo "[queue] fixed-inference probe old_ckpt steps=20 seed=${seed}" >> "$QUEUE_LOG"
    : > "$log"
    eval_ckpt "$OLD_CKPT" 3 20 "$seed" "$log"
    cat "$log" >> "$QUEUE_LOG"
    successes=$(parse_successes "$log")
    successes=${successes:-0}
    echo "[queue] fixed-inference old_ckpt seed=${seed} successes=${successes}/3" >> "$QUEUE_LOG"
    if [ "$successes" -ge 2 ]; then
      verify_log="train_logs/verify_dit12_tokenfix56_oldckpt_fixedinfer_steps20_seed${seed}_10trials_20260627.log"
      echo "[queue] fixed-inference probe reached >=2/3; launching 10-trial verification seed=${seed}" >> "$QUEUE_LOG"
      : > "$verify_log"
      eval_ckpt "$OLD_CKPT" 10 20 "$seed" "$verify_log"
      cat "$verify_log" >> "$QUEUE_LOG"
      successes=$(parse_successes "$verify_log")
      successes=${successes:-0}
      echo "[queue] fixed-inference old_ckpt verify seed=${seed} successes=${successes}/10" >> "$QUEUE_LOG"
    fi
  done
}

train_tokenfix_from_base() {
  echo "[queue] launching clean DIT12 training with exact 56 action tokens" >> "$QUEUE_LOG"
  "$TORCHRUN" --standalone --nnodes 1 --nproc-per-node 1 \
    ...
    --run_id_note VLA-Adapter-DIT12-tokenfix56-pure-uniformt-nodetach1500-20260627 \
...
eval_tokenfix_checkpoints() {
  local step seed ckpt log verify_log successes
  for step in 500 1000 1500; do
    ckpt="outputs/${RUN_ID}--${step}_chkpt"
    [ -d "$ckpt" ] || continue
    for seed in 0 3 7; do
      log="train_logs/dit12_tokenfix56_step${step}_steps20_seed${seed}_eval3_20260627.log"
      echo "[queue] eval tokenfix step=${step} seed=${seed}" >> "$QUEUE_LOG"
      : > "$log"
      eval_ckpt "$ckpt" 3 20 "$seed" "$log"
      cat "$log" >> "$QUEUE_LOG"
      successes=$(parse_successes "$log")
      successes=${successes:-0}
      echo "[queue] tokenfix step=${step} seed=${seed} successes=${successes}/3" >> "$QUEUE_LOG"
      if [ "$successes" -ge 2 ]; then
        verify_log="train_logs/verify_dit12_tokenfix56_step${step}_steps20_seed${seed}_10trials_20260627.log"
        echo "[queue] tokenfix candidate reached >=2/3; launching 10-trial verification step=${step} seed=${seed}" >> "$QUEUE_LOG"
        : > "$verify_log"
        eval_ckpt "$ckpt" 10 20 "$seed" "$verify_log"
        cat "$verify_log" >> "$QUEUE_LOG"
        successes=$(parse_successes "$verify_log")
        successes=${successes:-0}
        echo "[queue] tokenfix verify step=${step} seed=${seed} successes=${successes}/10" >> "$QUEUE_LOG"
      fi
    done
  done
}
...
"$PYTHON" -m py_compile prismatic/extern/hf/modeling_prismatic.py prismatic/vla/datasets/datasets.py
wait_for_gpu
run_fixed_inference_probe
wait_for_gpu
train_tokenfix_from_base
