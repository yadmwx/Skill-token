...
QUEUE_LOG=train_logs/queue_dit12_pure_gripfix_continue2000_20260624.log
TRAIN_LOG=train_logs/dit12_pure_gripfix_continue2000_20260624.log
OUT_BASE=outputs/configs+libero_object_no_noops+b16+lr-0.0002+lora-r32+dropout-0.0--image_aug--VLA-Adapter-DIT12-fullaction-pure-gripfix-nozeroinit-object-1000-20260623
RESUME_CHKPT="${OUT_BASE}--1000_chkpt"
FINAL_CHKPT="${OUT_BASE}--2000_chkpt"
...
echo "[queue] resuming pure DIT12 full-action gripper-fixed training from 1000 to 2000" >> "$QUEUE_LOG"
...
/home/xiaguanxiao/miniconda3/envs/vla-flow/bin/torchrun --standalone --nnodes 1 --nproc-per-node 1 \
  ...
  --run_id_note VLA-Adapter-DIT12-fullaction-pure-gripfix-nozeroinit-object-1000-20260623 \
  --run_id_override configs+libero_object_no_noops+b16+lr-0.0002+lora-r32+dropout-0.0--image_aug--VLA-Adapter-DIT12-fullaction-pure-gripfix-nozeroinit-object-1000-20260623 \
  --dit_num_blocks 12 \
  --flow_ratio 1.0 \
  ...
  --dit_flow_gripper_bce_weight 0.5 \
...
echo "[queue] resumed training finished, starting pure eval at default gripper threshold" >> "$QUEUE_LOG"
./eval.sh "$FINAL_CHKPT" \
...
echo "[queue] starting diagnostic pure eval at raw gripper threshold 0.9" >> "$QUEUE_LOG"
./eval.sh "$FINAL_CHKPT" \
  --task_suite libero_object --task_ids 0 --num_trials 3 --action_head_type DIT \
  --dit_num_blocks 12 --flow_ratio 1.0 --dit_num_inference_steps 20 --dit_num_inference_samples 1 \
  --dit_supervised_anchor_weight 0.0 --dit_anchor_blend 0.0 \
  ...
  --dit_zero_init_adaln False --dit_zero_init_output False \
  --use_adaptive_bridge True --bridge_mode adaptive --fixed_layer_index -1 \
  --debug_raw_gripper_threshold 0.9 \
  >> "$QUEUE_LOG" 2>&1
