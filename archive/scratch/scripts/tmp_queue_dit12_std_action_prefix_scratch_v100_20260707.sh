...
export PYTHONPATH=.:/autodl-fs/data/skill_depth/code/action_expert_prefix_trial/LIBERO/libero:/autodl-fs/data/skill_depth/code/action_expert_prefix_trial/robosuite:${PYTHONPATH:-}
...
log "queue start"
log "finetune_sha=$(sha256sum vla-scripts/finetune.py | awk '{print $1}')"
...
if [ -f "$STATUS" ] && [ "$(cat "$STATUS")" = 0 ]; then
  ...
  : > "$TRAIN_LOG"
  : > "$STATUS"
  log "launch scratch standard action_expert_prefix train label=$LABEL"
  wait_gpu
  set +e
