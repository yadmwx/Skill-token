...
def make_cfg(args):
    ...
    cfg = SimpleNamespace(
        ...
        flowmlp_anchor_num_layers=(12 if getattr(args, "match_depthcurriculum_seed9", False) else 0),
        flowmlp_anchor_hidden_dim=1024,
        flowmlp_use_latent_skill_token=bool(getattr(args, "match_depthcurriculum_seed9", False)),
        flowmlp_skill_use_direct_conditioning=(False if getattr(args, "match_depthcurriculum_seed9", False) else True),
        flowmlp_skill_assignment_mode=("soft" if getattr(args, "match_depthcurriculum_seed9", False) else "hard_gumbel"),
        flowmlp_skill_routing_mode=("query_key_soft" if getattr(args, "match_depthcurriculum_seed9", False) else "legacy"),
        flowmlp_skill_temperature=(0.7 if getattr(args, "match_depthcurriculum_seed9", False) else 1.0),
        flowmlp_time_embedding_mode=("continuous" if getattr(args, "match_depthcurriculum_seed9", False) else "legacy"),
        flowmlp_time_sampling_mode=("openpi_beta" if getattr(args, "match_depthcurriculum_seed9", False) else "uniform"),
        flowmlp_float32_path=bool(getattr(args, "match_depthcurriculum_seed9", False)),
        flowmlp_zero_init_output=(False if getattr(args, "match_depthcurriculum_seed9", False) else True),
        ...
        depth_utility_layers=args.depth_utility_layers,
        depth_utility_noise_repeats=args.depth_utility_noise_repeats,
...
@torch.no_grad()
def predict_batch(cfg, model, action_head, depth_interface, proprio_projector, processor, batch):
    ...
    if cfg.action_head_type.upper() == "DIT" and hasattr(action_head_obj, "predict_velocity"):
        ...
        candidate_layers = [int(x) for x in cfg.depth_utility_layers.split(",")]
        if min(candidate_layers) < 0 or max(candidate_layers) >= hidden_states.shape[1]:
            raise ValueError(
                f"candidate layers {candidate_layers} outside [0, {hidden_states.shape[1] - 1}]"
            )
        velocity_network = action_head_obj.velocity_network
        original_fixed_layer = int(velocity_network.fixed_layer_index)
        utility_repeats = []
        try:
            for repeat_idx in range(cfg.depth_utility_noise_repeats):
                generator = torch.Generator(device=target_actions.device)
                generator.manual_seed(int(cfg.seed * 100003 + repeat_idx * 1009))
                x0_u = torch.randn(
                ...
                t_u_view = t_u.view(batch_size, 1, 1)
                x_t_u = t_u_view * flow_x1 + (1.0 - t_u_view) * x0_u
                if getattr(action_head_obj.flow_cfg, "sample_target_t_mode", "relative") == "relative":
                    r_u = torch.zeros_like(t_u)
                else:
                    r_u = t_u
                target_velocity_u = flow_x1 - x0_u
                layer_losses = []
                for layer_idx in candidate_layers:
                    velocity_network.fixed_layer_index = int(layer_idx)
                    v_layer = action_head_obj.predict_velocity(
                        x_t_u,
                        t_u,
                        r_u,
                        hidden_states,
                    ...
                    error = (v_layer.float() - target_velocity_u.float()).pow(2)
                    error[..., :3] *= float(cfg.dit_flow_xyz_loss_weight)
                    error[..., 3:6] *= float(cfg.dit_flow_rot_loss_weight)
                    error[..., -1] *= float(cfg.dit_flow_gripper_loss_weight)
                    layer_losses.append(error.mean(dim=(1, 2)))
                utility_repeats.append(torch.stack(layer_losses, dim=-1))
        finally:
            velocity_network.fixed_layer_index = original_fixed_layer
        depth_utility_info = {
            "candidate_layers": candidate_layers,
            "losses": torch.stack(utility_repeats, dim=0).detach().cpu(),
        }
    ...
    decoded_prompts = processor.tokenizer.batch_decode(input_ids.detach().cpu(), skip_special_tokens=True)
    return (
        pred.float().detach().cpu(),
        target_actions.float().detach().cpu(),
        split_info,
        teacher_flow_info,
        depth_utility_info,
        decoded_prompts,
...
def main():
    ...
    parser.add_argument("--match_depthcurriculum_seed9", type=str2bool, default=False)
    parser.add_argument("--depth_utility_layers", default="1,5,9,13,24")
    parser.add_argument("--depth_utility_noise_repeats", type=int, default=4)
    parser.add_argument("--depth_utility_temperature", type=float, default=0.05)
    ...
    for _ in range(args.num_batches):
        batch = next(iterator)
        pred, target, split_info, teacher_flow_info, depth_utility_info, batch_prompts = predict_batch(
            cfg,
            model,
            action_head,
            depth_interface,
        ...
        if depth_utility_info:
            depth_utility_infos.append(depth_utility_info)
        decoded_prompts.extend(batch_prompts)
    ...
    if depth_utility_infos:
        candidate_layers = depth_utility_infos[0]["candidate_layers"]
        utility = torch.cat([info["losses"] for info in depth_utility_infos], dim=1).float()
        mean_per_sample = utility.mean(dim=0)
        avg_loss = mean_per_sample.mean(dim=0)
        best = mean_per_sample.argmin(dim=-1)
        counts = torch.bincount(best, minlength=len(candidate_layers))
        ref_pos = candidate_layers.index(24)
        oracle = mean_per_sample.min(dim=-1).values
        reference = mean_per_sample[:, ref_pos]
        improvement = reference - oracle
        repeat_best = utility.argmin(dim=-1)
        stable_fraction = (repeat_best == best.unsqueeze(0)).float().mean(dim=0)
        all_repeat_agree = (repeat_best == repeat_best[:1]).all(dim=0).float().mean()
        temperature = float(args.depth_utility_temperature)
        teacher = torch.softmax(-mean_per_sample / temperature, dim=-1)
        entropy = -(teacher * teacher.clamp_min(1e-12).log()).sum(dim=-1)
        effective_layers = entropy.exp()
        print("DEPTH_UTILITY_LAYERS", ",".join(map(str, candidate_layers)))
        print("DEPTH_UTILITY_AVG_LOSS", ",".join(f"{x:.6f}" for x in avg_loss.tolist()))
        print("DEPTH_UTILITY_BEST_COUNTS", ",".join(map(str, counts.tolist())))
        print("DEPTH_UTILITY_BEST_FRACTIONS", ",".join(f"{x:.6f}" for x in (counts.float()/best.numel()).tolist()))
        print(f"DEPTH_UTILITY_H24_LOSS {reference.mean().item():.6f}")
        print(f"DEPTH_UTILITY_ORACLE_LOSS {oracle.mean().item():.6f}")
        print(f"DEPTH_UTILITY_ORACLE_IMPROVEMENT_MEAN {improvement.mean().item():.6f}")
        print(f"DEPTH_UTILITY_ORACLE_IMPROVEMENT_MEDIAN {improvement.median().item():.6f}")
        print(f"DEPTH_UTILITY_ORACLE_IMPROVED_FRACTION {(improvement > 1e-8).float().mean().item():.6f}")
        print(f"DEPTH_UTILITY_TEACHER_ENTROPY {entropy.mean().item():.6f}")
        print(f"DEPTH_UTILITY_EFFECTIVE_LAYERS {effective_layers.mean().item():.6f}")
        print(f"DEPTH_UTILITY_REPEAT_MATCH_FRACTION {stable_fraction.mean().item():.6f}")
        print(f"DEPTH_UTILITY_ALL_REPEATS_AGREE {all_repeat_agree.item():.6f}")

        prompt_groups = {}
        for idx, text in enumerate(decoded_prompts):
            low = text.lower()
            if "bbq sauce" in low or "barbecue sauce" in low:
                key = "task3_bbq"
            elif "ketchup" in low or "tomato sauce" in low:
                key = "task4_ketchup"
            elif "milk" in low and "basket" in low:
                key = "task7_milk"
            elif "orange juice" in low:
                key = "task9_orange"
            else:
                key = "other"
            prompt_groups.setdefault(key, []).append(idx)
        for key, indices in sorted(prompt_groups.items()):
            idx = torch.tensor(indices, dtype=torch.long)
            group_loss = mean_per_sample.index_select(0, idx)
            group_best = group_loss.argmin(dim=-1)
            group_counts = torch.bincount(group_best, minlength=len(candidate_layers))
            group_teacher = torch.softmax(-group_loss / temperature, dim=-1).mean(dim=0)
            print(
                f"DEPTH_UTILITY_GROUP {key} n={len(indices)} "
                f"best_counts={','.join(map(str, group_counts.tolist()))} "
                f"teacher={','.join(f'{x:.6f}' for x in group_teacher.tolist())}"
            )
        if args.output_json:
            payload = {
                "candidate_layers": candidate_layers,
                "losses": utility.tolist(),
