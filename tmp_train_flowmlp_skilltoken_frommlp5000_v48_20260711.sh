...
LABEL=flowmlp_skilltoken_frommlp5000_10k_v48_20260711
RUN_ID=FlowMLP-skilltoken-frommlp5000-10k-v48-20260711
...
run_eval(){
  ...
  "$PY" experiments/robot/libero/run_libero_eval.py \
    ...
    --flowmlp_skill_temperature 1.0 --flowmlp_skill_entropy_weight 0.0 \
    --flowmlp_num_inference_steps 5 --flowmlp_num_inference_samples 8 \
...
"$TORCHRUN" --standalone --nnodes 1 --nproc_per_node 1 vla-scripts/finetune.py \
  ...
  --run_id_override configs+libero_object_no_noops+b8+lr-0.0002+lora-r64+dropout-0.0--image_aug--${RUN_ID} \
