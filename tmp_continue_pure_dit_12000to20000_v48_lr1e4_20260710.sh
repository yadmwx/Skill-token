...
LABEL=dit12_h896_std_action_prefix_scratch_continue12000to20000_v48_lr1e4_20260710
RUN_ID=DIT12-h896-std-action-prefix-scratch-continue12000to20000-v48-lr1e4-20260710
RESUME_CKPT=/autodl-fs/data/skill_depth/code/action_expert_prefix_trial/outputs/configs+libero_object_no_noops+b4+lr-0.0002+lora-r32+dropout-0.0--image_aug--DIT12-h896-std-action-prefix-scratch-b4ga4-12000-v48-20260708--12000_chkpt
...
wait_gpu(){
  while true; do
    ...
    log "gpu free=${free}MiB util=${util}% jobs=${jobs}"
...
run_eval(){
  local step=$1
  local chkpt=$2
  local eval_log=train_logs/${LABEL}_step${step}_hardtasks_eval5.log
  log "launch eval step=${step} chkpt=${chkpt}"
  wait_gpu
  set +e
  "$PY" experiments/robot/libero/run_libero_eval.py \
    --pretrained_checkpoint "$chkpt" --action_head_type DIT \
    --task_suite_name libero_object --task_ids 3,4,7,9 --num_trials_per_task 5 \
    ...
    --dit_supervised_anchor_weight 0.0 --dit_anchor_blend 0.0 --dit_anchor_blend_was_set True --dit_inference_residual_scale 1.0 \
    --dit_anchor_gripper_weight 1.0 --dit_anchor_gripper_bce_weight 0.0 --dit_flow_xyz_loss_weight 1.0 \
    --dit_flow_rot_loss_weight 1.0 --dit_flow_gripper_loss_weight 1.0 --dit_flow_gripper_bce_weight 0.0 \
    --dit_sample_t_mode_flow beta --dit_detach_flow_conditioning False --dit_disable_inference_anchor True --dit_pure_inference True \
    ...
    --dit_condition_mode task_only --dit_condition_injection_mode action_expert_prefix --dit_include_prompt_tokens True \
    --dit_task_token_mode vision_prompt --dit_zero_init_adaln True --dit_zero_init_output False \
    --use_proprio True --num_images_in_input 2 --use_film False --use_minivlm True --use_pro_version True \
    --use_wandb False --center_crop True --seed 7 > "$eval_log" 2>&1
  rc=$?; set -e
  log "eval step=${step} rc=${rc} successes=$(success_count "$eval_log")"
}

log "queue start; source=$RESUME_CKPT"
if [ ! -f "$STATUS" ] || [ "$(cat "$STATUS")" != 0 ]; then
  ...
  "$TORCHRUN" --standalone --nnodes 1 --nproc_per_node 1 vla-scripts/finetune.py \
    ...
    --resume_load_training_state False --train_from checkpoint --resum_vla_path "$RESUME_CKPT" --resume_step 12000 \
    ...
    --num_images_in_input 2 --use_film False --learning_rate 0.0001 --use_constant_lr True --lr_warmup_steps 200 \
    --max_steps 20000 --save_freq 2000 --save_latest_checkpoint_only False --image_aug True --use_wandb False --wandb_log_freq 50 \
    --dit_hidden_dim 896 --dit_num_blocks 12 --dit_num_inference_steps 20 --dit_num_inference_samples 1 \
    ...
    --run_id_override configs+libero_object_no_noops+b8+lr-0.0001+lora-r32+dropout-0.0--image_aug--${RUN_ID} \
    --batch_size 8 --grad_accumulation_steps 2 > "$TRAIN_LOG" 2>&1
  rc=$?; echo "$rc" > "$STATUS"; set -e
  log "train finished rc=${rc}"
  if [ "$rc" -ne 0 ]; then exit "$rc"; fi
fi

for step in 14000 16000 18000 20000; do
  chkpt=$(find outputs -maxdepth 1 -type d -name "*${RUN_ID}--${step}_chkpt" | sort | tail -1)
  [ -n "$chkpt" ] && run_eval "$step" "$chkpt" || log "missing checkpoint step=${step}"
done
log "queue done"
