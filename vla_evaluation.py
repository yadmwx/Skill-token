...
class DualSystemCalvinEvaluation(CalvinBaseModel):
    ...
    def step(self, obs, instruction, step):
        ...
        # Checkpoints produced by the RLDS trainer may use the dataset key
        # `calvin_abc`, while the historical CALVIN evaluator uses
        # `calvin_abc_rlds`. Accept both spellings at rollout time.
        stats_key = "calvin_abc_rlds" if "calvin_abc_rlds" in self.OFT.norm_stats else "calvin_abc"
        proprio_norm_stats = self.OFT.norm_stats[stats_key]['proprio']
