...
QUEUE_LOG=train_logs/queue_dit12_pure_uniformt_nodetach1500_20260624.log
TRAIN_LOG=train_logs/dit12_pure_uniformt_nodetach1500_20260624.log
RUN_ID=configs+libero_object_no_noops+b16+lr-0.0002+lora-r32+dropout-0.0--image_aug--VLA-Adapter-DIT12-pure-uniformt-nodetach1500-20260624
...
while true; do
  ...
  active_jobs=$( { pgrep -af 'finetune.py|run_libero_eval.py|tmp_queue_dit12_pure_clean_base1000' || true; } | wc -l | tr -d ' ')
  ts=$(date '+%F %T')
...
echo "[queue] launching pure DIT12 uniform-t no-detach training, no anchor, no gate" >> "$QUEUE_LOG"
...
/home/xiaguanxiao/miniconda3/envs/vla-flow/bin/torchrun --standalone --nnodes 1 --nproc-per-node 1 \
  ...
  --run_id_note VLA-Adapter-DIT12-pure-uniformt-nodetach1500-20260624 \
...
for step in 500 1000 1500; do
  ...
  eval_log="train_logs/dit12_pure_uniformt_nodetach1500_step${step}_eval3_20260624.log"
  verify_log="train_logs/verify_dit12_pure_uniformt_nodetach1500_step${step}_10trials_20260624.log"
