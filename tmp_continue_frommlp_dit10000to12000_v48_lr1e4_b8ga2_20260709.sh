...
LABEL=dit12_h896_std_action_prefix_pure_frommlp5000_continue10000to12000_v48_lr1e4_b8ga2_20260709
RUN_ID=DIT12-h896-std-action-prefix-pure-frommlp5000-continue10000to12000-v48-lr1e4-b8ga2-20260709
QUEUE_LOG=train_logs/queue_${LABEL}.log
TRAIN_LOG=train_logs/${LABEL}.log
STATUS=train_logs/${LABEL}.status
RESUME_CKPT=outputs/configs+libero_object_no_noops+b8+lr-0.0002+lora-r32+dropout-0.0--image_aug--DIT12-h896-std-action-prefix-pure-frommlp5000-resume5000to10000-v48-b8ga2-20260709--10000_chkpt
...
wait_gpu(){
  while true; do
    ...
    if [ "$free" -gt 30000 ] && [ "$util" -lt 20 ] && [ "$jobs" -eq 0 ]; then break; fi
    sleep 60
  done
}
...
run_eval(){
  local chkpt=$1
  local eval_log=train_logs/${LABEL}_step12000_hardtasks_eval5.log
  local eval_status=train_logs/${LABEL}_step12000_hardtasks_eval5.status
...
"$TORCHRUN" --standalone --nnodes 1 --nproc_per_node 1 \
  vla-scripts/finetune.py \
    ...
    --resum_vla_path "$RESUME_CKPT" \
    --resume_step 10000 \
    --resume_load_training_state False \
    --vlm_training freeze_lora \
    --action_head_type DIT \
    --use_lora True --lora_rank 64 --lora_dropout 0.0 --freeze_vlm True --merge_lora_during_training True \
    ...
    --max_steps 12000 --save_freq 2000 --save_latest_checkpoint_only False --image_aug True \
    ...
    --run_id_note "$RUN_ID" --run_id_override configs+libero_object_no_noops+b8+lr-0.0001+lora-r64+dropout-0.0--image_aug--${RUN_ID} \
...
CHKPT=$(find outputs -maxdepth 1 -type d -name "*${RUN_ID}--12000_chkpt" | sort | tail -1)
if [ -z "$CHKPT" ]; then
  log "missing checkpoint"
  exit 2
fi
run_eval "$CHKPT"
log "queue done"
