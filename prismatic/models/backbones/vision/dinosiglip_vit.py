...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(
        ...
        dino_ckpt = os.environ.get("PRISMATIC_DINO_CHECKPOINT")
        siglip_ckpt = os.environ.get("PRISMATIC_SIGLIP_CHECKPOINT")
        self.dino_featurizer: VisionTransformer = timm.create_model(
            self.dino_timm_path_or_url, pretrained=(dino_ckpt is None), checkpoint_path=dino_ckpt,
            num_classes=0, img_size=self.default_image_size
        )
        self.dino_featurizer.eval()

        self.siglip_featurizer: VisionTransformer = timm.create_model(
            self.siglip_timm_path_or_url, pretrained=(siglip_ckpt is None), checkpoint_path=siglip_ckpt,
            num_classes=0, img_size=self.default_image_size
        )
