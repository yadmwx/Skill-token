...
mkdir -p train_logs
QUEUE_LOG=train_logs/queue_dit12_statecond_sqrtgroup_nozeroinit_proprioadd_statepos_20260623.log
TRAIN_LOG=train_logs/dit12_statecond_sqrtgroup_nozeroinit_proprioadd_statepos_20260623.log
OUT_BASE=outputs/configs+libero_object_no_noops+b16+lr-0.0002+lora-r32+dropout-0.0--image_aug--VLA-Adapter-DIT12-statecond-sqrtgroup-nozeroinit-proprioadd-statepos-object-1000-20260623
CHKPT_DIR="${OUT_BASE}--1000_chkpt"

echo "[queue] waiting for proprioadd queue to finish" >> "$QUEUE_LOG"
while pgrep -f 'tmp_queue_dit12_statecond_sqrtgroup_nozeroinit_proprioadd_20260623.sh' >/dev/null; do
  ts=$(date '+%F %T')
  echo "[$ts] waiting for tmp_queue_dit12_statecond_sqrtgroup_nozeroinit_proprioadd_20260623.sh" >> "$QUEUE_LOG"
  sleep 120
done
...
echo "[queue] launching DIT12 state-conditioning sqrt_group no-zero-init proprio-add training with explicit chunk positions" >> "$QUEUE_LOG"
...
/home/xiaguanxiao/miniconda3/envs/vla-flow/bin/torchrun --standalone --nnodes 1 --nproc-per-node 1 \
  ...
  --run_id_note VLA-Adapter-DIT12-statecond-sqrtgroup-nozeroinit-proprioadd-statepos-object-1000-20260623 \
  --dit_num_blocks 12 \
  --flow_ratio 1.0 \
