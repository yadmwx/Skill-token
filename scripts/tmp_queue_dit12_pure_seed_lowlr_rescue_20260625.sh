...
RUN_ID=configs+libero_object_no_noops+b16+lr-0.0002+lora-r32+dropout-0.0--image_aug--VLA-Adapter-DIT12-pure-uniformt-nodetach-lowlr-resume1000-1300-20260625
QUEUE_LOG=train_logs/queue_dit12_pure_seed_lowlr_rescue_20260625.log
TRAIN_LOG=train_logs/dit12_pure_uniformt_nodetach_lowlr_resume1000_1300_20260625.log
...
run_probe() {
  local out_log=train_logs/probe_dit12_pure_uniformt_nodetach_step1000_actions_20260625.log
...
preflight_seed_sweep() {
  ...
  for steps in 10 20 30; do
    for seed in 0 1 2 3 7 11 23; do
      log="train_logs/dit12_pure_uniformt_nodetach_step1000_steps${steps}_seed${seed}_eval3_20260625.log"
      ...
      if [ "$successes" -ge 2 ]; then
        verify_log="train_logs/verify_dit12_pure_uniformt_nodetach_step1000_steps${steps}_seed${seed}_10trials_20260625.log"
...
preflight_grouped_action_token_sweep() {
  ...
  for steps in 10 20 30; do
    for seed in 0 1 2 3 7 11 23; do
      log="train_logs/dit12_pure_uniformt_nodetach_step1000_groupctx_steps${steps}_seed${seed}_eval3_20260625.log"
      ...
      if [ "$successes" -ge 2 ]; then
        verify_log="train_logs/verify_dit12_pure_uniformt_nodetach_step1000_groupctx_steps${steps}_seed${seed}_10trials_20260625.log"
...
train_lowlr_resume() {
  ...
  "$TORCHRUN" --standalone --nnodes 1 --nproc-per-node 1 \
    ...
    --run_id_note VLA-Adapter-DIT12-pure-uniformt-nodetach-lowlr-resume1000-1300-20260625 \
...
eval_lowlr_checkpoints() {
  ...
  for step in 1100 1200 1300; do
    ...
    for seed in 7 0 11; do
      log="train_logs/dit12_pure_uniformt_nodetach_lowlr_resume1000_1300_step${step}_seed${seed}_eval3_20260625.log"
      ...
      if [ "$successes" -ge 2 ]; then
        verify_log="train_logs/verify_dit12_pure_uniformt_nodetach_lowlr_resume1000_1300_step${step}_seed${seed}_10trials_20260625.log"
