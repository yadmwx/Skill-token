...
RUN_ID=configs+libero_object_no_noops+b16+lr-0.0002+lora-r32+dropout-0.0--image_aug--VLA-Adapter-DIT12-tokenfix56-statecond-sqrtpos1000-20260627
QUEUE_LOG=train_logs/queue_dit12_tokenfix56_statecond_sqrtpos1000_20260627.log
TRAIN_LOG=train_logs/dit12_tokenfix56_statecond_sqrtpos1000_20260627.log
...
wait_for_gpu() {
  ...
  while true; do
    local used util active_jobs tokenfix_jobs ts
    ...
    tokenfix_jobs=$(ps -eo args= | awk '/[t]mp_queue_dit12_tokenfix_56actiontokens_20260627.sh/ {n++} END {print n + 0}')
    ts=$(date '+%F %T')
    echo "[$ts] gpu_used=${used}MiB gpu_util=${util}% active_jobs=${active_jobs} tokenfix_jobs=${tokenfix_jobs}" >> "$QUEUE_LOG"
    if [ "$used" -le 7000 ] && [ "$util" -le 20 ] && [ "$active_jobs" -eq 0 ] && [ "$tokenfix_jobs" -eq 0 ]; then
      break
    fi
    sleep 60
  done
...
train_statecond_from_base() {
  echo "[queue] launching clean DIT12 state-conditioned training with exact 56 action tokens" >> "$QUEUE_LOG"
  "$TORCHRUN" --standalone --nnodes 1 --nproc-per-node 1 \
    ...
    --run_id_note VLA-Adapter-DIT12-tokenfix56-statecond-sqrtpos1000-20260627 \
...
eval_statecond_checkpoints() {
  local step seed ckpt log verify_log successes
  for step in 500 1000; do
    ckpt="outputs/${RUN_ID}--${step}_chkpt"
    [ -d "$ckpt" ] || continue
    for seed in 0 3 7; do
      log="train_logs/dit12_tokenfix56_statecond_step${step}_steps20_seed${seed}_eval3_20260627.log"
      echo "[queue] eval statecond step=${step} seed=${seed}" >> "$QUEUE_LOG"
      : > "$log"
      eval_ckpt "$ckpt" 3 20 "$seed" "$log"
      cat "$log" >> "$QUEUE_LOG"
      successes=$(parse_successes "$log")
      successes=${successes:-0}
      echo "[queue] statecond step=${step} seed=${seed} successes=${successes}/3" >> "$QUEUE_LOG"
      if [ "$successes" -ge 2 ]; then
        verify_log="train_logs/verify_dit12_tokenfix56_statecond_step${step}_steps20_seed${seed}_10trials_20260627.log"
        echo "[queue] statecond candidate reached >=2/3; launching 10-trial verification step=${step} seed=${seed}" >> "$QUEUE_LOG"
        : > "$verify_log"
        eval_ckpt "$ckpt" 10 20 "$seed" "$verify_log"
        cat "$verify_log" >> "$QUEUE_LOG"
        successes=$(parse_successes "$verify_log")
        successes=${successes:-0}
        echo "[queue] statecond verify step=${step} seed=${seed} successes=${successes}/10" >> "$QUEUE_LOG"
      fi
    done
  done
}
...
"$PYTHON" -m py_compile prismatic/extern/hf/modeling_prismatic.py prismatic/vla/datasets/datasets.py prismatic/models/flow_matching_head.py prismatic/models/action_heads.py
wait_for_gpu
train_statecond_from_base
wait_for_gpu
eval_statecond_checkpoints
